...
fun BlueBar(
    ...
    BoxWithConstraints(
        modifier = modifier
            .pointerInput(Unit) {
                awaitEachGesture {
                    ...
                    val newBlue = ((newX - radiusPx) / (width - 2 * radiusPx)).coerceIn(0f, 1f)

                    // Оновлюємо колір
                    val rgb = (color.value as RGBColor)
                    color.value = rgb.copy(blue = (newBlue * 255).toInt())
                    ...
                    drag(down.id) { change ->
                        ...
                        val newDragBlue = ((newDragX - radiusPx) / (width - 2 * radiusPx)).coerceIn(0f, 1f)

                        // Оновлюємо колір під час перетягування
                        color.value = rgb.copy(blue = (newDragBlue * 255).toInt())
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            ...
            // Створюємо градієнт для синього кольору
            val blueColors = (0..255).map { blueValue ->
                colorRGBdecimalOf(red = rgb.red, green = rgb.green, blue = blueValue).asComposeColor()
            }

            // Малюємо фон з градієнтом
            drawRoundRect(
                brush = Brush.horizontalGradient(blueColors),
                topLeft = Offset(0f, 0f),
                size = Size(widthPx.toPx(), maxHeight.toPx()),
                cornerRadius = CornerRadius(radiusPx.toPx(), radiusPx.toPx())
            )

            // Позиція поточного синього кольору
            val currentBlueX = (rgb.blue / 255f) * (widthPx.toPx() - 2 * radiusPx.toPx()) + radiusPx.toPx()

            // Малюємо індикатор
            val centerX = pressOffset?.x ?: currentBlueX
