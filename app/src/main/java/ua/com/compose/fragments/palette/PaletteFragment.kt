...
import ua.com.compose.dialogs.DialogInput
...
class PaletteFragment : BaseMvvmFragment(R.layout.module_other_color_pick_fragment_palette) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModule.colors.nonNull().observe(viewLifecycleOwner) {
            ...
            if(it?.isNotEmpty() == true) {
                ...
                (activity as BaseMvpView).setupBottomMenu(mutableListOf(btnAddColor))
            } else {
        ...
        viewModule.palettes.nonNull().observe(viewLifecycleOwner) {
            ...
            binding.lstColors.isVisible = !pallets.isEmpty()
    ...
    private fun createPallet() {
        val request = DialogInput.show(fm = this.childFragmentManager, title = requireContext().getString(R.string.module_other_color_pick_pallet_name), text = Settings.defaultPaletteName(context = requireContext(), withIncrement = false))
        this.childFragmentManager.setFragmentResultListener(request, viewLifecycleOwner) { _, bundle ->
