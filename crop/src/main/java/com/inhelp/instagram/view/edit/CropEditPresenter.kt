...
class CropEditPresenter(val context: Context, val transferObject: TransferObject) : BaseMvpPresenterImpl<CropEditView>() {
    ...
    fun onLoadImage() {
        image = transferObject.image ?: image
        image?.let { bitmap ->
    ...
    fun pressDone() {
        ...
        transferObject.image = Util.merge(scaledBackground, image)
